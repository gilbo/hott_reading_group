...
Section Chapter_2_Exercises.
  ...
  Proof.
    ...
    rewrite transport_eq_f in ppeq.
  ...
  Theorem ap_spaireq_again {A A' : UU} (B : A -> UU) (B' : A' -> UU)
  ...
  Proof.
    ...
    generalize feqv; clear feqv;
      ...
      rewrite p; clear p; clear fwd;
        intro feqv; destruct feqv as ((fR,req),(fL,leq)); simpl.
    (* case equiv_refl *)
    left; apply (spaireq idpath);
        apply paireq; unfold idfun,funcomp in *; simpl;
          [ apply (spaireq (funext req)) | apply (spaireq (funext leq)) ];
          apply funext; intro; apply bool_is_set.
    (* case bneg_refl *)
    right; apply (spaireq idpath); apply paireq; simpl;
    [ apply (spaireq (funext(λ x, inv (bneg_inv _)
                                       ∙ ap bneg (req x))))
    | apply (spaireq (funext(λ x, ap fL (inv (bneg_inv _))
                                      ∙ leq (bneg x))))
